...
class TestOffers < Test::Unit::TestCase
  ...
  def test_multiple_offers
    ...
    offers = offersService.getOffers('player1', 'campaign2', '1')
    assert(offers.length == 2)
    assertOffer(offers[0], 266892, 'Clash of Lords 2: Ehrenkampf', 'http://cdn3.sponsorpay.com/app_icons/19476/small_mobile_icon.png', 17781)
    assertOffer(offers[1], 356777, 'Gratis Prepaid-SIM', 'http://cdn3.sponsorpay.com/assets/15196/offerwall-02_square_60.jpg', 76772)
  end
  ...
  private
  def assertOffer(offer, id, title, thumbnailLowRes, payout)
    assert(offer.id == id)
